<?php
namespace Waxedphp\Terminal\Parsers;
    ...
    */
    ...
    function getIt($cmd) {
      ...
      foreach($a as $b) {
        ...
        if (isset($b['pipe'])) $d['pipe'] = $b['pipe'];
        $key = null;
        foreach($b['values'] as $k => $v) {
          if ($k == 0) {
            $d['command'] = $v['value'];
          } else if (strpos($v['value'], '--')===0) {
            $key = substr($v['value'],2);
            $d['params'][$key] = $key;
          } else if (strpos($v['value'], '-')===0) {
            if (!isset($d['switches'])) $d['switches'] = [];
            $sw = substr($v['value'],1);
            ...
            $d['params'][$key] = $v['value'];
            $key = null;
          } else {
            $d['params'][] = $v['value'];
          }
        }
        if (isset($d['switches'])) $d['switches'] = array_keys($d['switches']);
        if (empty($d['params'])) unset($d['params']);
        if (!$d['pipe']) unset($d['pipe']);
        $data[] = $d;
      }
